...
// --- EVENT DETAIL MODAL (Replicated from personal_agenda.js) ---

window.openHomepageEventDetails = async function (event) {
    console.log("[Homepage] Opening details for event:", event);
    ...
    const modalId = 'hp-event-detail-modal';
    ...
    const headerHtml = `
        <div class="modal-header" style="flex: 0 0 auto; display:flex !important; justify-content:space-between !important; align-items:flex-start !important; margin-bottom: 0; padding-bottom: 1rem; border-bottom: 1px solid var(--glass-border); width: 100%;">
            <div style="text-align: left !important; flex: 1;">
                <h3 style="margin:0; font-size:1.1rem; color:var(--text-primary); font-family:var(--font-titles); line-height: 1.3;">${modalTitle}</h3>
                <div style="font-size: 0.8rem; color: var(--text-secondary); margin-top: 4px;">${dateStr}</div>
            </div>
            <button class="icon-btn" onclick="closeHomepageEventModal('${modalId}')" style="margin-left: 1rem; flex: 0 0 auto;">
    ...
    if (isAppt) {
        bodyContent = `
            <div id="hp-appt-loader" style="display: flex; justify-content: center; padding: 2rem;">
                <div class="loader"></div>
            </div>
            <div id="hp-appt-content" style="display: none; flex-direction: column; gap: 1rem; padding-top: 1rem;"></div>
        `;
    } else {
        // Simple Booking Info (We might not need async fetch for bookings if data is enough, but usually safer to use what we have)
        bodyContent = `
            <div style="padding-top: 1rem;">
                <div style="background:var(--bg-secondary); padding:1rem; border-radius:8px; border-left: 4px solid #3b82f6;">
                    ...
                    <div style="font-size:0.9rem; color:var(--text-secondary); display:flex; align-items:center; gap:0.5rem;">
                         <span class="material-icons-round" style="font-size:16px;">schedule</span> ${timeStr}
                    </div>
                </div>
                <div style="margin-top: 1rem;">
                    <h4 style="font-size:0.7rem; text-transform:uppercase; color:var(--text-tertiary);">Cliente</h4>
                    <div style="font-weight:500;">${event.client || '-'}</div>
                </div>
            </div>
        `;
    }
    ...
    const modalHtml = `
        <div class="system-modal active" id="${modalId}" style="z-index: 10000; display: flex; justify-content: center; align-items: center; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); backdrop-filter: blur(4px);">
            <div class="system-modal-content" style="
                display: flex; 
                flex-direction: column; 
                max-width: ${isAppt ? '700px' : '450px'};
                width: 90%; 
                max-height: 85vh; 
                background: white; 
                border-radius: 16px; 
                ...
                <div class="modal-footer" style="padding-top: 1rem; margin-top: auto; border-top: 1px solid var(--glass-border); display: flex; justify-content: flex-end;">
                     <button class="btn btn-secondary" onclick="closeHomepageEventModal('${modalId}')">Chiudi</button>
                </div>
            </div>
        </div>
    `;
    ...
    // FETCH DETAILS IF APPT
    if (isAppt) {
        try {
            // Lazy load collabs if needed
            if (!state.collaborators || state.collaborators.length === 0) {
                await fetchCollaborators();
            }

            const fullEvent = await fetchAppointment(event.id);
            if (!fullEvent) throw new Error("Appt not found");

            const container = document.getElementById('hp-appt-content');
            const loader = document.getElementById('hp-appt-loader');

            if (container && loader) {
                // Render Details
                const clientName = fullEvent.orders?.clients?.business_name || fullEvent.client_name || '-';
                const orderRef = fullEvent.orders ? `#${fullEvent.orders.order_number} ${fullEvent.orders.title}` : '';
                const location = fullEvent.location || 'Nessun luogo';
                const mode = fullEvent.mode || '-';
                const note = fullEvent.note || '';

                container.innerHTML = `
                    <!-- Info Card -->
                    <div style="background: white; border: 1px solid var(--glass-border); border-radius: 12px; padding: 1rem; border-left: 4px solid #a855f7;">
                        <div style="font-size: 1rem; font-weight: 700; color: var(--text-primary);">${fullEvent.title || 'Appuntamento'}</div>
                        <div style="font-size: 0.8rem; color: var(--text-tertiary); margin-top: 4px;">${timeStr}</div>
                    </div>

                    <!-- Griglia Dettagli -->
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                        <div class="glass-card" style="padding: 0.75rem;">
                            <div style="font-size: 0.6rem; text-transform: uppercase; color: var(--text-tertiary);">Cliente</div>
                            <div style="font-weight: 600; font-size: 0.85rem;">${clientName}</div>
                        </div>
                         <div class="glass-card" style="padding: 0.75rem;">
                            <div style="font-size: 0.6rem; text-transform: uppercase; color: var(--text-tertiary);">Commessa</div>
                            <div style="font-weight: 600; font-size: 0.85rem; color: var(--brand-blue);">${orderRef || '-'}</div>
                        </div>
                        <div class="glass-card" style="padding: 0.75rem;">
                            <div style="font-size: 0.6rem; text-transform: uppercase; color: var(--text-tertiary);">Luogo</div>
                            <div style="font-size: 0.85rem;">${location}</div>
                        </div>
                         <div class="glass-card" style="padding: 0.75rem;">
                            <div style="font-size: 0.6rem; text-transform: uppercase; color: var(--text-tertiary);">Modalità</div>
                            <div style="font-size: 0.85rem;">${mode}</div>
                        </div>
                    </div>

                    <!-- Note -->
                    <div class="glass-card" style="padding: 1rem;">
                         <div style="font-size: 0.65rem; font-weight: 600; color: var(--text-tertiary); text-transform: uppercase; margin-bottom: 0.5rem;">Note</div>
                         <div style="font-size: 0.85rem; color: var(--text-secondary); white-space: pre-wrap;">${note || 'Nessuna nota.'}</div>
                    </div>
                `;
            ...
            console.error("Error fetching appt details:", e);
            const container = document.getElementById('hp-appt-content');
            if (container) container.innerHTML = '<div style="color:red;">Errore caricamento dati appuntamento.</div>';
        }
    }
};
