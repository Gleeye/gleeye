...
export async function renderSpaceView(container, spaceId) {
    ...
    try {
        ...
        container.querySelector('#add-activity-btn')?.addEventListener('click', () => {
            container.querySelector('#add-hub-dropdown').classList.add('hidden');
            import('/js/features/pm/components/hub_drawer.js?v=8020').then(mod => mod.openHubDrawer(null, spaceId, null, 'attivita'));
        });

        container.querySelector('#add-task-btn')?.addEventListener('click', () => {
            container.querySelector('#add-hub-dropdown').classList.add('hidden');
            import('/js/features/pm/components/hub_drawer.js?v=8020').then(mod => mod.openHubDrawer(null, spaceId, null, 'task'));
        });
