            ...
            if (currentTab === 'overview') {
                ...
                content.innerHTML = `
                    ...
                    ${filteredLogs.length === 0 ? `<div style="padding: 3rem; text-align: center; color: var(--text-tertiary); font-style: italic;">Nessuna attività recente registrata per questo cluster.</div>` : filteredLogs.slice(0, 30).map(l => {
                        ...
                        return `
                        <div style="display: flex; gap: 1rem; padding: 1rem; border-bottom: 1px solid var(--bg-primary); animation: fadeIn 0.3s ease; cursor:pointer;" onclick="import('./components/hub_drawer.js?v=8017').then(m => m.openHubDrawer('${l.item_ref}', '${l.space_ref}'))">
