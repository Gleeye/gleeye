...
export async function renderSpaceOverview(container, spaceId, items, space) {
    ...
    try {
        ...
        // Link Drive (cloud_links)
        const docs = (space.cloud_links || []).slice(0, 4);

        // Documenti veri (doc_pages incluso voice memo report e whiteboard)
        let recentDocPages = [];
        try {
            const { data: ds } = await supabase.from('doc_spaces').select('id').eq('space_ref', spaceId);
        ...
        } catch (e) { console.warn('[space_overview] doc_pages fetch failed', e); }
        ...
        container.innerHTML = `
            ...
            <div class="overview-dash">
                ...
                <div class="dash-box" style="grid-row: span 2;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                        ${urgencies.length === 0 ? `<div class="dash-empty">Nessuna scadenza imminente</div>` : urgencies.slice(0,5).map(u => `
                            <div class="urg-item cursor-pointer" onclick="import('./hub_drawer.js?v=8026').then(m => m.openHubDrawer('${u.id}', '${spaceId}'))">
                ...
                <div class="dash-box">
                    <div class="dash-title">
                        <div style="display:flex; align-items:center; gap:8px;"><span class="material-icons-round" style="color:var(--text-primary); font-size: 1.3rem;">description</span> Documenti Recenti</div>
                        <span class="material-icons-round" style="cursor:pointer;" title="Vai a tutti i documenti" onclick="document.querySelector('.tab-btn[data-view=docs]').click()">arrow_forward</span>
                    </div>
        ...
        // Wire click su Documenti Recenti → fullscreen viewer
        container.querySelectorAll('.doc-page-item').forEach(el => {
            el.onclick = async () => {
                ...
                try {
                    ...
                    console.error('[space_overview] open doc error', e);
