    ...
    if (currentView === 'kanban') {
        ...
        cards.forEach(card => {
            card.onclick = (e) => {
                ...
                import('/js/features/pm/components/hub_drawer.js?v=8022').then(mod => mod.openHubDrawer(card.dataset.id, spaceId));
            };
        ...
        container.querySelectorAll('.tree-row').forEach(row => {
            row.onclick = (e) => {
                ...
                if (!e.target.closest('.tree-toggle') && !e.target.closest('.add-child-btn')) {
                    import('/js/features/pm/components/hub_drawer.js?v=8022').then(mod => mod.openHubDrawer(row.dataset.id, spaceId));
                }
            };
        ...
        menu.querySelectorAll('.menu-item').forEach(it => {
            it.onclick = async () => {
                ...
                if (type === 'project') {
                    ...
                    import('./hub_drawer.js?v=8022').then(mod => mod.openHubDrawer(null, spaceId, btn.dataset.parent, type));
                }
                menu.remove();
            };
        });
