...
export async function openAppointmentDrawer(inputAppointment, contextId = null, contextType = 'order', options = {}) {
        ...
        const html = `
            ...
            ">
                <div id="hub-drawer" style="
                    ...
                    background: var(--bg-card, white); box-shadow: -10px 0 40px rgba(0,0,0,0.2);
    ...
    const render = () => {
        if (viewMode) {
            ...
            drawer.innerHTML = `
                <div class="drawer-header" style="padding: 1.25rem 2rem; border-bottom: 1px solid var(--border-default, #e2e8f0); display: flex; justify-content: space-between; align-items: center; background: var(--bg-card, white); flex-shrink: 0; position: sticky; top: 0; z-index: 10;">
                ...
                <div class="drawer-body" style="flex: 1; overflow-y: auto; padding: 2rem 2rem;">
                    <div style="display: flex; flex-direction: column; gap: 1.75rem; max-width: 580px; margin: 0 auto;">
                        ...
                        <div style="height: 1px; background: var(--bg-tertiary, #f1f5f9); margin: 0.5rem 0;"></div>
                ...
                return `<div style="display: flex; align-items: center; gap: 8px; padding: 4px 10px; background: var(--bg-card, white); border: 1.2px solid var(--border-default, #e2e8f0); border-radius: 20px; font-size: 0.8rem; font-weight: 600;">
                ...
                <div class="drawer-footer" style="padding: 1.25rem 2rem; border-top: 1px solid var(--border-default, #e2e8f0); display: flex; justify-content: flex-end; gap: 0.75rem; background: var(--bg-card, white); flex-shrink: 0; position: sticky; bottom: 0; z-index: 10;">
                    <button type="button" class="secondary-btn" id="edit-btn" style="padding: 0.6rem 1.25rem; font-weight: 500; border-radius: 8px; border: 1.2px solid var(--border-default, #e2e8f0); background: var(--bg-card, white); color: var(--text-secondary); cursor: pointer; transition: all 0.2s;">Modifica</button>
        ...
        drawer.innerHTML = `
            ...
            <div class="drawer-body" style="flex: 1; overflow-y: auto; padding: 1.5rem 2rem;">
                <form id="appt-form" style="display: flex; flex-direction: column; gap: 1.75rem; max-width: 580px; margin: 0 auto;">
                    ...
                    <div class="form-group">
                        ...
                        <div style="position: relative;">
                            <div id="context-picker-trigger" style="
                                padding: 0 14px; height: 44px; background: var(--bg-tertiary, #f8fafc); border: 1.2px solid var(--border-default, #e2e8f0); border-radius: 10px; 
                            ...
                            <div id="context-picker-dropdown" class="hidden glass-card" style="position: absolute; top: calc(100% + 6px); left: 0; width: 100%; z-index: 1000; background: var(--bg-card, white); border: 1.2px solid var(--border-default, #e2e8f0); border-radius: 12px; box-shadow: 0 12px 48px rgba(0,0,0,0.12); padding: 10px;">
                                <div style="display: flex; gap: 8px; margin-bottom: 8px;"><input type="text" id="context-search" placeholder="Filtra..." class="input-modern" style="flex: 1; font-size: 0.85rem; height: 36px; padding: 0 12px; border-radius: 8px; border: 1.2px solid var(--border-default, #e2e8f0);"><button type="button" id="clear-context-btn" title="Rimuovi associazione" style="background: var(--bg-tertiary, #f8fafc); border: 1.2px solid var(--border-default, #e2e8f0); border-radius: 8px; padding: 0 8px; cursor: pointer; color: var(--text-tertiary);"><span class="material-icons-round" style="font-size: 18px;">backspace</span></button></div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" required value="${formState.title}" class="input-modern" style="height: 44px; padding: 0 14px; border-radius: 10px; font-weight: 500; font-size: 1rem; border: 1.2px solid var(--border-default, #e2e8f0); width: 100%; box-sizing: border-box; background: var(--bg-card, white);">
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                        <div class="form-group">
                            ...
                            <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap; padding: 6px; border: 1.2px solid var(--border-default, #e2e8f0); border-radius: 10px; min-height: 44px; background: var(--bg-card, white);">
                                ${formState.participants.internal.map((p, idx) => {
            const collab = state.collaborators?.find(c => c.id === p.collaborator_id);
            return `<div class="pill" style="display: flex; align-items: center; gap: 6px; background: var(--bg-tertiary, #f8fafc); padding: 3px 8px; border-radius: 6px; font-size: 0.8rem; font-weight: 500; border: 1.2px solid var(--border-default, #e2e8f0); color: var(--text-primary);">
                                        <span>${collab?.full_name && collab.full_name !== 'null' ? collab.full_name : joinNames(collab?.first_name, collab?.last_name) || 'User'}</span>
                                ...
                                <div style="position: relative;">
                                    ...
                                    <div id="int-picker" class="hidden glass-card" style="position: absolute; top: calc(100% + 6px); left: 0; min-width: 220px; z-index: 1000; max-height: 250px; overflow-y: auto; background: var(--bg-card, white); border: 1.2px solid var(--border-default, #e2e8f0); padding: 6px; border-radius: 12px; box-shadow: 0 12px 48px rgba(0,0,0,0.12);">
            ...
            return `<div class="pill" style="display: flex; align-items: center; gap: 6px; background: var(--bg-tertiary, #f8fafc); padding: 3px 8px; border-radius: 6px; font-size: 0.8rem; font-weight: 500; border: 1.2px solid var(--border-default, #e2e8f0); color: var(--text-primary);">
                                ...
                                <div style="position: relative;">
                                    ...
                                    <div id="cli-picker" class="hidden glass-card" style="position: absolute; top: calc(100% + 6px); left: 0; min-width: 220px; z-index: 1000; max-height: 250px; overflow-y: auto; background: var(--bg-card, white); border: 1.2px solid var(--border-default, #e2e8f0); padding: 6px; border-radius: 12px; box-shadow: 0 12px 48px rgba(0,0,0,0.12);">
            ...
            return `<div class="type-pill ${selected ? 'selected' : ''}" data-id="${t.id}" style="padding: 6px 14px; border-radius: 20px; border: 1.5px solid ${t.color}; color: ${selected ? 'white' : t.color}; background: ${selected ? t.color : 'transparent'}; cursor: pointer; font-size: 0.8rem; font-weight: 700; transition: all 0.2s;">${t.name}</div>`;
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" name="start_time" required value="${formState.start_time}" class="input-modern" style="height: 44px; padding: 0 14px; border-radius: 10px; font-weight: 500; font-size: 0.95rem; border: 1.2px solid var(--border-default, #e2e8f0); width: 100%; box-sizing: border-box; background: var(--bg-card, white);">
                        </div>
                        <div class="form-group">
                            <label style="display: block; font-size: 0.85rem; font-weight: 500; color: var(--text-secondary); margin-bottom: 0.5rem;">Fine</label>
                            <input type="datetime-local" name="end_time" required value="${formState.end_time}" class="input-modern" style="height: 44px; padding: 0 14px; border-radius: 10px; font-weight: 500; font-size: 0.95rem; border: 1.2px solid var(--border-default, #e2e8f0); width: 100%; box-sizing: border-box; background: var(--bg-card, white);">
                        </div>
                    </div>
                    ...
                    <div class="recurrence-section">
                        <button type="button" id="toggle-recurrence-btn" style="
                            width: 100%; padding: 0.75rem 1rem; background: var(--bg-tertiary, #f8fafc); border: 1.2px solid var(--border-default, #e2e8f0); 
                        ...
                        <div id="recurrence-settings-content" class="hidden" style="
                            padding: 1.5rem; background: #fff; border: 1.2px solid var(--border-default, #e2e8f0); border-top: none; 
                        ...
                        ">
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.25rem;">
                                <div>
                                    ...
                                    <select name="rec_freq" style="height: 38px; width: 100%; border-radius: 8px; border: 1.2px solid var(--border-default, #e2e8f0); padding: 0 8px; font-size: 0.9rem;">
                                ...
                                <div>
                                    ...
                                    <div style="display: flex; gap: 8px;">
                                        <input type="number" name="rec_interval" value="1" min="1" style="width: 50px; height: 38px; text-align: center; border-radius: 8px; border: 1.2px solid var(--border-default, #e2e8f0); padding: 0;">
                                        <select name="rec_unit" style="flex: 1; height: 38px; border-radius: 8px; border: 1.2px solid var(--border-default, #e2e8f0); padding: 0 8px; font-size: 0.85rem;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.25rem;">
                                <div>
                                    ...
                                    <div id="form-rec-start-trigger" style="cursor: pointer; display: flex; align-items: center; gap: 8px; height: 38px; border-radius: 8px; border: 1.2px solid var(--border-default, #e2e8f0); padding: 0 10px; font-size: 0.9rem; background: var(--bg-card, white);">
                                ...
                                <div>
                                    ...
                                    <div id="form-rec-until-trigger" style="cursor: pointer; display: flex; align-items: center; gap: 8px; height: 38px; border-radius: 8px; border: 1.2px solid var(--border-default, #e2e8f0); padding: 0 10px; font-size: 0.9rem; background: var(--bg-card, white);">
                            ...
                            <div style="height: 1px; background: var(--bg-tertiary, #f1f5f9); margin: 1.25rem 0;"></div>
                            ...
                            <div style="display: flex; flex-direction: column; gap: 1rem;">
                                <div style="display: flex; align-items: center; justify-content: space-between;">
                                    ...
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        ...
                                        <input type="number" name="rec_limit_count" value="10" min="1" style="width: 60px; height: 32px; text-align: center; border-radius: 6px; border: 1.2px solid var(--border-default, #e2e8f0);">
                                    </div>
                                </div>
                                ...
                                <div style="display: flex; align-items: center; justify-content: space-between;">
                                    ...
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        ...
                                        <input type="number" name="rec_advance_count" value="5" min="1" style="width: 60px; height: 32px; text-align: center; border-radius: 6px; border: 1.2px solid var(--border-default, #e2e8f0);">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="location" value="${formState.location}" class="input-modern" placeholder="Indirizzo o Link..." style="height: 44px; padding: 0 14px; border-radius: 10px; font-weight: 500; font-size: 0.95rem; border: 1.2px solid var(--border-default, #e2e8f0); width: 100%; box-sizing: border-box; background: var(--bg-card, white);">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="note" class="input-modern" style="min-height: 100px; padding: 12px; font-size: 0.95rem; line-height: 1.6; border-radius: 10px; border: 1.2px solid var(--border-default, #e2e8f0); width: 100%; box-sizing: border-box; resize: vertical; background: var(--bg-card, white);" placeholder="Aggiungi dettagli...">${formState.note}</textarea>
                    </div>

                </form>
            </div>

            <div class="drawer-footer" style="padding: 1.25rem 2rem; border-top: 1px solid var(--border-default, #e2e8f0); display: flex; justify-content: flex-end; gap: 0.75rem; background: var(--bg-card, white); flex-shrink: 0; position: sticky; bottom: 0; z-index: 10; align-items: center;">
                ${!isCreate ? `<button type="button" id="delete-btn" class="icon-btn" style="width: 38px; height: 38px; border-radius: 8px; display: flex; align-items: center; justify-content: center; background: var(--bg-card, white); border: 1.2px solid var(--color-error-soft, #fee2e2); color: var(--color-error, #ef4444); cursor: pointer; transition: all 0.2s; margin-right: auto;"><span class="material-icons-round" style="font-size: 1.2rem;">delete_outline</span></button>` : ''}
                <button type="button" class="secondary-btn" id="cancel-btn" style="padding: 0.6rem 1.25rem; font-weight: 500; border-radius: 8px; border: 1.2px solid var(--border-default, #e2e8f0); background: var(--bg-card, white); color: var(--text-secondary); cursor: pointer; transition: all 0.2s;">Annulla</button>
        ...
        if (ctxTrigger && ctxDropdown) {
            ...
            function renderContextOptions(filter = '') {
                ...
                if (clients.length > 0 && !formState.client_id) {
                    ...
                    clients.forEach(c => {
                        html += `<div class="ctx-option" data-id="${c.id}" data-type="client" style="padding: 10px 12px; border-radius: 8px; cursor: pointer; display: flex; align-items: center; gap: 8px;"><span class="material-icons-round" style="font-size: 16px; color: var(--color-warning, #f59e0b);">person</span><span style="font-size: 0.85rem;">${c.business_name}</span></div>`;
                    });
                }
                ...
                if (orders.length > 0) {
                    html += `<div style="padding: 12px 8px 4px; font-size: 0.65rem; color: var(--text-tertiary); font-weight: 700; border-top: 1px solid var(--border-default, #e2e8f0); margin-top: 4px;">COMMESSE ATTIVE</div>`;
                    orders.forEach(o => {
