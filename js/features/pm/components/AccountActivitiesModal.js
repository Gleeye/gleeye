...
export async function openAccountActivitiesModal(orderId, spaceId) {
    ...
    addBtn.addEventListener('click', async () => {
        if (currentTab === 'attivita' || currentTab === 'task') {
            const { openHubDrawer } = await import('/js/features/pm/components/hub_drawer.js?v=8020');
...
function renderTreeNodes(nodes, spaceId) {
    ...
    return nodes.map(node => {
        ...
        return `
            <div class="tree-node">
                <div class="tree-row" onclick="import('/js/features/pm/components/hub_drawer.js?v=8020').then(mod => mod.openHubDrawer('${node.id}', '${spaceId}'))">
...
async function renderItems(container, spaceId, filterType, items = null) {
    try {
        ...
        container.innerHTML = `
            ...
            return `
                        <div class="activity-card" style="opacity: ${isDone ? '0.7' : '1'}; cursor: pointer;" onclick="import('/js/features/pm/components/hub_drawer.js?v=8020').then(mod => mod.openHubDrawer('${item.id}', '${spaceId}'))">
