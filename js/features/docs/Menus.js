...
export const SlashMenu = {
    ...
    renderItems(items) {
        ...
        if (items.length === 0) {
            this.element.innerHTML = '<div style="padding:12px; color:var(--text-tertiary, #94a3b8); font-size:13px;">Nessuna corrispondenza</div>';
            return;
        }
        ...
        items.forEach((item, index) => {
            ...
            div.innerHTML = `
                <div style="display:flex; align-items:center; justify-content:center; height:24px;">
                    <span class="material-icons-round" style="color:var(--text-secondary, #64748b); font-size:20px;">${item.icon}</span>
                </div>
                <div style="display:flex; flex-direction:column; min-width:0;">
                    <span style="font-weight:500; color:var(--text-primary, #334155); font-size:14px; line-height:1.4;">${item.label}</span>
                    <span style="font-size:12px; color:var(--text-tertiary, #94a3b8); line-height:1.3; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${item.desc}</span>
                </div>
            `;
            ...
            div.onmouseenter = () => {
                ...
                Array.from(this.element.children).forEach((child, i) => {
                    child.style.backgroundColor = i === index ? 'var(--bg-tertiary, #f1f5f9)' : 'transparent';
                });
            };
...
export const BlockActionMenu = {
    ...
    init() {
        ...
        this.element.style.backgroundColor = 'var(--bg-card, white)';
        this.element.style.borderRadius = '6px';
        this.element.style.boxShadow = '0 4px 12px rgba(0,0,0,0.15)';
        this.element.style.border = '1px solid var(--border-default, #e2e8f0)';
        this.element.style.display = 'none';
    ...
    render() {
        const actions = [
            { label: 'Delete', icon: 'delete', action: 'delete', color: 'var(--color-error, #ef4444)' },
        ...
        actions.forEach(action => {
            ...
            div.style.color = action.color || 'var(--text-primary, #334155)';
            div.style.width = '100%';
            div.style.boxSizing = 'border-box';
            ...
            div.onmouseenter = () => div.style.backgroundColor = 'var(--bg-tertiary, #f8fafc)';
