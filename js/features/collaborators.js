...
import { upsertCollaborator, fetchPayments, fetchAssignments, fetchPassiveInvoices, fetchAvailabilityRules, saveAvailabilityRules, fetchAvailabilityOverrides, upsertAvailabilityOverride, deleteAvailabilityOverride, fetchCollaboratorServices, fetchBookingItemCollaborators } from '../modules/api.js?v=317';
    ...
    if (!document.getElementById('collaborator-modal')) {
        ...
        document.getElementById('delete-collab-btn')?.addEventListener('click', async () => {
            ...
            if (await window.showConfirm(`Sei sicuro di voler eliminare ${c.full_name}? Questa azione è irreversibile.`)) {
                try {
                    const { deleteCollaborator } = await import('../modules/api.js?v=317');
...
window.impersonateCollaborator = async (collaboratorId) => {
    ...
    if (await window.showConfirm(`Vuoi impersonare ${c.full_name}? Vedrai l'interfaccia come se fossi questo collaboratore.`)) {
        ...
        // Update Sidebar
        import('./layout.js?v=317').then(({ updateSidebarVisibility, renderSidebarProfile }) => {
