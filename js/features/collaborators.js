...
import { openDepartmentManager } from './settings.js?v=155';
import { upsertCollaborator, fetchPayments, fetchAssignments, fetchPassiveInvoices, fetchAvailabilityRules, saveAvailabilityRules, fetchAvailabilityOverrides, upsertAvailabilityOverride, deleteAvailabilityOverride, fetchCollaboratorServices, fetchBookingItemCollaborators } from '../modules/api.js?v=155';
import { loadAvailabilityIntoContainer } from './availability_manager.js?v=155';
    ...
    if (!document.getElementById('collaborator-modal')) {
        ...
        document.getElementById('delete-collab-btn')?.addEventListener('click', async () => {
            ...
            if (await window.showConfirm(`Sei sicuro di voler eliminare ${c.full_name}? Questa azione è irreversibile.`)) {
                try {
                    const { deleteCollaborator } = await import('../modules/api.js?v=155');
...
window.impersonateCollaborator = async (collaboratorId) => {
    ...
    if (await window.showConfirm(`Vuoi impersonare ${c.full_name}? Vedrai l'interfaccia come se fossi questo collaboratore.`)) {
        ...
        // Update Sidebar
        import('./layout.js?v=155').then(({ updateSidebarVisibility, renderSidebarProfile }) => {
...
window.sendMagicLink = async (email) => {
    ...
    if (await window.showConfirm(`Vuoi inviare un Magic Link di accesso a ${email}?`)) {
        try {
            const { supabase } = await import('../modules/config.js?v=155');
...
async function initAvailabilityTab(collaboratorId) {
    ...
    try {
        await loadAvailabilityIntoContainer(container, collaboratorId);
        loading.style.display = 'none';
        container.classList.remove('hidden');
    } catch (err) {
        console.error("Tab Availability Error:", err);
