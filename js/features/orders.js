...
import { upsertPayment, deletePayment, upsertOrder, updateOrder, updateOrderEconomics, fetchPayments, fetchOrders, fetchAssignments, fetchCollaborators, fetchServices, addOrderAccount, removeOrderAccount, addOrderContact, removeOrderContact, fetchOrderContacts } from '../modules/api.js?v=123';
import { openPaymentModal } from './payments.js?v=123';
...
export async function renderOrderDetail(container, orderId) {
    ...
    if (!state.collaboratorServices || state.collaboratorServices.length === 0) {
        const { fetchCollaboratorServices } = await import('../modules/api.js?v=123');
...
window.filterAssignmentCollaborators = () => {
    ...
    if (!state.collaborators) {
        ...
        import('../modules/api.js?v=123').then(({ fetchCollaborators }) => fetchCollaborators());
        // Show temp message
...
window.loadCollaboratorServicesForAssignment = async () => {
    ...
    try {
        ...
        const { fetchServices } = await import('../modules/api.js?v=123' + Date.now());
...
window.saveAssignmentMultiStep = async () => {
    ...
    try {
        ...
        const { upsertAssignment, upsertCollaboratorService, fetchCollaboratorServices, fetchAssignments, upsertPayment, fetchPayments } = await import('../modules/api.js?v=123' + Date.now());
        const { calculateProposedAssignmentPayments } = await import('./assignments.js?v=123');
