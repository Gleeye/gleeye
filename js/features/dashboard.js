...
export function renderDashboard(container) {
    const renderContent = () => {
        ...
        // Don't error out if elements aren't there (e.g. during fast navigation)
        ...
        resetBtn.addEventListener('mouseenter', () => resetBtn.style.background = 'rgba(139, 92, 246, 0.1)');
    ...
    const onDataLoaded = () => {
        // If container is still in DOM, re-render
        if (document.body.contains(container)) {
            renderContent();
        } else {
            // Cleanup if view was destroyed
