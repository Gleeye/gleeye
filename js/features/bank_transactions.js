...
import { renderReadOnlyView, switchToEditMode } from './bank_transaction_readonly.js?v=155';
...
export function initCategoryManagerModal() {
    ...
    document.getElementById('new-category-form').addEventListener('submit', async (e) => {
        ...
        try {
            const { upsertTransactionCategory } = await import('../modules/api.js?v=155');
...
window.deleteCategory = async (id) => { if (await window.showConfirm("Sei sicuro di voler eliminare questa categoria?", { type: 'danger' })) { try { const { deleteTransactionCategory } = await import('../modules/api.js?v=155'); await deleteTransactionCategory(id); reloadCatList(); } catch (e) { window.showAlert(e.message, 'error'); } } };
