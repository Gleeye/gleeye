    ...
    const renderGrid = () => {
        ...
        return filtered.map(s => {
            ...
            <div class="card service-card" onclick="window.location.hash = '#sap-service-detail/${s.id}'" style="cursor:pointer; padding: 1.25rem; display: flex; flex-direction: column; gap: 0.75rem; position: relative; height: 100%;">
                ...
                <div style="margin-top: auto; padding-top: 0.75rem; border-top: 1px solid var(--glass-border); display: flex; flex-wrap: wrap; gap: 0.25rem; align-items: center; justify-content: space-between;">
                    ...
                    ${s.ai_doc_status === 'ready' ? `<span style="font-size: 0.65rem; padding: 2px 8px; border-radius: 10px; background: rgba(16,185,129,0.1); color: #10b981; font-weight: 700; display:flex; align-items:center; gap:3px;"><span class="material-icons-round" style="font-size:0.75rem;">auto_awesome</span>Docs AI</span>` : ''}
                    ${s.ai_doc_status === 'generating' ? `<span style="font-size: 0.65rem; padding: 2px 8px; border-radius: 10px; background: rgba(59,130,246,0.1); color: var(--brand-blue); font-weight: 700; display:flex; align-items:center; gap:3px;"><span class="material-icons-round" style="font-size:0.75rem;">hourglass_top</span>Generando…</span>` : ''}
                </div>
            </div>
        `;
        }).join('');
        ...
        <div class="animate-fade-in" style="max-width: 1400px; margin: 0 auto; padding: 1.5rem;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; background: var(--bg-secondary); padding: 1.5rem 2rem; border-radius: 16px; border: 1px solid var(--glass-border);">
                ...
                <div style="display: flex; align-items: center; gap: 0.75rem;">
                    ...
                    <button onclick="window.openSapDocGenerator('${service.id}')" style="display:flex; align-items:center; gap:0.5rem; padding:0.6rem 1.25rem; border-radius:12px; border:none; cursor:pointer; background: var(--brand-gradient); color:white; font-weight:700; font-size:0.85rem; box-shadow: 0 4px 12px rgba(99,102,241,0.3); transition: opacity 0.2s;" onmouseover="this.style.opacity='0.85'" onmouseout="this.style.opacity='1'">
                        ...
                        ${service.ai_doc_status === 'ready' ? 'Rigenera Docs AI' : 'Genera documentazione AI'}
                    </button>
                </div>
            </div>
            ...
            <div id="sap-ai-section" style="margin-top: 1.5rem; background: var(--bg-secondary); border: 1px solid var(--glass-border); border-radius: 16px; overflow: hidden;">
                <div onclick="document.getElementById('sap-ai-section-body').classList.toggle('hidden'); this.querySelector('.expand-icon').textContent = document.getElementById('sap-ai-section-body').classList.contains('hidden') ? 'expand_more' : 'expand_less';" style="display: flex; align-items: center; justify-content: space-between; padding: 1.25rem 1.5rem; cursor: pointer; user-select: none;">
                    <div style="display: flex; align-items: center; gap: 0.75rem;">
                        <div style="width: 32px; height: 32px; border-radius: 8px; background: var(--brand-gradient); display: flex; align-items: center; justify-content: center; color: white;">
                        ...
                        <div>
                            ...
                            <span style="margin-left: 0.75rem; font-size: 0.7rem; padding: 2px 8px; border-radius: 8px; font-weight: 700;
                                ${service.ai_doc_status === 'ready' ? 'background: rgba(16,185,129,0.12); color: #10b981;' :
                                  ...
                                  service.ai_doc_status === 'error' ? 'background: rgba(239,68,68,0.12); color: #ef4444;' :
                                  'background: var(--bg-tertiary); color: var(--text-tertiary);'}">
                                ${ service.ai_doc_status === 'ready' ? '✓ Docs generati' :
                                   service.ai_doc_status === 'generating' ? '⟳ Generando...' :
                                   service.ai_doc_status === 'error' ? '✕ Errore' : 'Da compilare' }
                            </span>
                        </div>
                    </div>
                    <span class="material-icons-round expand-icon" style="color: var(--text-tertiary);">${service.value_proposition ? 'expand_less' : 'expand_more'}</span>
                </div>

                <div id="sap-ai-section-body" class="${service.value_proposition ? '' : 'hidden'}" style="padding: 0 1.5rem 1.5rem 1.5rem; display: flex; flex-direction: column; gap: 1.25rem;">
                    <style>
                        #sap-ai-section-body.hidden { display: none; }
                        .ai-field-label { font-size: 0.7rem; font-weight: 800; text-transform: uppercase; color: var(--text-tertiary); margin-bottom: 0.35rem; display: block; }
                        .ai-field-input { width: 100%; padding: 0.65rem 0.85rem; border-radius: 10px; border: 1px solid var(--glass-border); background: var(--bg-color); color: var(--text-primary); font-size: 0.9rem; font-family: inherit; resize: vertical; box-sizing: border-box; }
                        .ai-field-input:focus { outline: none; border-color: var(--brand-blue); box-shadow: 0 0 0 3px rgba(59,130,246,0.12); }
                    </style>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                        <div>
                            <label class="ai-field-label">Value Proposition <span style="color: var(--text-tertiary); font-weight: 400; text-transform: none;">(cosa risolve, per chi)</span></label>
                            <textarea id="sap-ai-value-prop" class="ai-field-input" rows="3" placeholder="Es. Crea il tuo podcast professionale senza set fisico...">${service.value_proposition || ''}</textarea>
                        </div>
                        <div>
                            <label class="ai-field-label">Target Customer <span style="color: var(--text-tertiary); font-weight: 400; text-transform: none;">(segmento, contesto)</span></label>
                            <textarea id="sap-ai-target" class="ai-field-input" rows="3" placeholder="Es. Imprenditori, consulenti, esperti di settore...">${service.target_customer || ''}</textarea>
                        </div>
                    </div>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                        <div>
                            <label class="ai-field-label">Cosa include il pacchetto</label>
                            <textarea id="sap-ai-includes" class="ai-field-input" rows="3" placeholder="Es. 1× Setup remoto + 3h registrazione + Editing + Distribuzione">${(service.package_includes || []).map(i => typeof i === 'string' ? i : (i.label || '')).join('\n')}</textarea>
                            <div style="font-size: 0.7rem; color: var(--text-tertiary); margin-top: 0.25rem;">Una voce per riga</div>
                        </div>
                        <div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                        <div>
                            <label class="ai-field-label">Tempo di consegna (giorni)</label>
                            <input type="number" id="sap-ai-delivery" class="ai-field-input" min="1" placeholder="Es. 14" value="${service.delivery_time_days || ''}">
                        </div>
                        <div>
                            <label class="ai-field-label">Team richiesto</label>
                            <input type="text" id="sap-ai-team" class="ai-field-input" placeholder="Es. 1 tecnico audio + 1 editor" value="${service.team_required || ''}">
                        </div>
                    </div>
...
window.saveAiFields = async (serviceId) => {
    const get = id => document.getElementById(id)?.value?.trim() || null;
    const parseLines = id => (document.getElementById(id)?.value || '').split('\n').map(l => l.trim()).filter(Boolean);

    const payload = {
        value_proposition: get('sap-ai-value-prop'),
        target_customer: get('sap-ai-target'),
        package_includes: parseLines('sap-ai-includes'),
        pricing_tiers: parseLines('sap-ai-pricing'),
        delivery_time_days: parseInt(get('sap-ai-delivery')) || null,
        team_required: get('sap-ai-team'),
    };

    const { error } = await supabase.from('core_services').update(payload).eq('id', serviceId);
    if (error) { await window.showAlert('Errore salvataggio: ' + error.message, 'error'); return; }

    const svc = state.sapServices.find(s => s.id === serviceId);
    if (svc) Object.assign(svc, payload);

    await window.showAlert('Dati salvati', 'success');
};
