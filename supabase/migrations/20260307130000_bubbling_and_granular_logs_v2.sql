-- 1. Helper function for recursive item children (bubbling)
CREATE OR REPLACE FUNCTION public.fn_get_pm_item_descendants(root_id UUID)
...
-- 2. Extend Documents to support Item Context
ALTER TABLE public.doc_pages 
ADD COLUMN IF NOT EXISTS item_ref UUID REFERENCES public.pm_items(id) ON DELETE CASCADE;

-- 3. Update Logger to handle doc_pages reference chain
...
BEGIN
    ...
    -- Space Ref (with parent resolution)
    ...
    -- Order Ref (with hierarchical resolution)
...
-- Register doc_pages in the Activity Registry
INSERT INTO public.pm_activity_registry (table_name, template_insert, insert_action_name, template_update, update_action_name, track_columns, order_ref_source, space_ref_source, item_ref_source)
VALUES (
    'doc_pages', 
    'Nuovo documento creato: {title}', 
    'document_created', 
    'Documento rinominato: {title}', 
    'document_updated',
    ARRAY['title'], 
    'space_ref.space_ref.ref_ordine', 
    'space_ref.space_ref', 
    'item_ref'
)
ON CONFLICT (table_name) DO UPDATE SET 
  ...
  update_action_name = EXCLUDED.update_action_name,
...
-- Update Task Rules (pm_items)
UPDATE public.pm_activity_registry 
SET 
  track_columns = ARRAY['status', 'priority', 'start_date', 'due_date', 'notes', 'title', 'cloud_links'],
  column_templates = '{
    ...
    "notes": "Descrizione attività aggiornata",
    "title": "Rinominata in: {title}",
    "cloud_links": "Risorse cloud aggiornate"
  }'::jsonb
WHERE table_name = 'pm_items';

-- Triggers
DROP TRIGGER IF EXISTS trg_doc_pages_generic_log ON public.doc_pages;
CREATE TRIGGER trg_doc_pages_generic_log AFTER INSERT OR UPDATE ON public.doc_pages FOR EACH ROW EXECUTE FUNCTION public.fn_app_activity_logger();
