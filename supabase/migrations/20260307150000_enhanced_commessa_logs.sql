-- 1. Refined Logger to support hierarchical space resolution from order_id
...
BEGIN
    ...
    IF (_final_log_count = 0 OR TG_OP != 'UPDATE') AND _template IS NOT NULL THEN
        ...
        IF _description LIKE '%{%' THEN
            FOR _col IN SELECT jsonb_object_keys(_new_json) LOOP
                _description := REPLACE(_description, '{' || _col || '}', COALESCE(_new_json->>_val, ''));
            END LOOP;
        END IF;
...
-- 2. Register Appointments
INSERT INTO public.pm_activity_registry (table_name, template_insert, insert_action_name, template_update, update_action_name, track_columns, column_templates, order_ref_source, space_ref_source, client_ref_source)
VALUES (
    'appointments', 
    'Nuovo appuntamento creato: {title}', 
    'appointment_created', 
    'Aggiornato appuntamento: {title}', 
    'appointment_updated',
    ARRAY['status', 'start_time', 'title'],
    '{
        "status": "Stato appuntamento cambiato in {new_value}",
        "start_time": "Data appuntamento spostata al {start_time}",
        "title": "Appuntamento rinominato in: {title}"
    }'::jsonb,
    'order_id', 
    'order_id.space_ref', 
    'client_id'
...
DROP TRIGGER IF EXISTS trg_appointments_generic_log ON public.appointments;
CREATE TRIGGER trg_appointments_generic_log AFTER INSERT OR UPDATE ON public.appointments FOR EACH ROW EXECUTE FUNCTION public.fn_app_activity_logger();

-- 3. Register Assignments (Incarichi)
INSERT INTO public.pm_activity_registry (table_name, template_insert, insert_action_name, template_update, update_action_name, track_columns, column_templates, order_ref_source, space_ref_source, client_ref_source)
VALUES (
    'assignments', 
    'Creato incarico: {description}', 
    'assignment_created', 
    'Aggiornato incarico: {description}', 
    'assignment_updated',
    ARRAY['status', 'collaborator_id'],
    '{
        "status": "Stato incarico cambiato in {new_value}",
        "collaborator_id": "Collaboratore incarico aggiornato"
    }'::jsonb,
    'order_id', 
    'order_id.space_ref', 
    'order_id.client_id'
...
-- 4. Refine Space (Commessa) Granular Templates
UPDATE public.pm_activity_registry 
SET 
  track_columns = ARRAY['status', 'name', 'default_pm_user_ref'],
  column_templates = '{
    "status": "Avanzamento commessa: {new_value}",
    "name": "Commessa rinominata in: {name}",
    "default_pm_user_ref": "Assegnato nuovo Project Manager alla commessa"
  }'::jsonb
