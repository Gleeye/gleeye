-- 1. Add is_account_level to pm_activity_logs
ALTER TABLE public.pm_activity_logs ADD COLUMN IF NOT EXISTS is_account_level BOOLEAN DEFAULT false;

-- 2. Update Generic Logger to resolve the flag
        ...
        FOR _col IN SELECT jsonb_object_keys(_diff_json) LOOP
            ...
            IF _col_template IS NOT NULL THEN
                ...
                INSERT INTO public.pm_activity_logs (
                    actor_ref, action_type, description, space_ref, item_ref, order_ref, client_ref, is_account_level, metadata
                ) VALUES (
...
-- 3. Refine Orders tracking
UPDATE public.pm_activity_registry 
SET 
  track_columns = ARRAY['status_works', 'title', 'offer_status', 'total_amount_tax_excluded', 'pm_user_id'],
  column_templates = '{
    ...
    "pm_user_id": "Assegnato nuovo PM commerciale/operativo all''ordine"
  }'::jsonb
