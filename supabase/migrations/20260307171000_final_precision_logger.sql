...
BEGIN
    ...
    IF (_final_log_count = 0 OR TG_OP != 'UPDATE') AND _template IS NOT NULL THEN
        ...
        _description := REPLACE(_description, '{body}', _new_json->>'body'); 
        _description := REPLACE(_description, '{title}', _new_json->>'title'); 
...
-- 3. THE "PRECISION" REGISTRY RE-REVOLUTION
...
-- Ensure we track the right columns for assignments
UPDATE public.pm_activity_registry 
SET track_columns = ARRAY['status', 'collaborator_id', 'total_amount']
WHERE table_name = 'assignments';

-- Assignees
INSERT INTO public.pm_activity_registry (table_name, track_insert, template_insert, item_ref_source, space_ref_source)
VALUES ('pm_item_assignees', true, 'ha assegnato {entity} a {user_name}', 'pm_item_ref', 'pm_item_ref.space_ref')
ON CONFLICT (table_name) DO UPDATE SET template_insert = EXCLUDED.template_insert;
