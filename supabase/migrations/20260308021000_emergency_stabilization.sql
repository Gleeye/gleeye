-- RESTORE AND STABILIZE LOGGER
BEGIN;

-- 1. Restore standard creation log template
UPDATE public.pm_activity_registry 
SET template_insert = 'ha creato l''attività {entity}'
WHERE table_name = 'pm_items';

-- 2. Clean the registry for assignees to avoid redundant phrases
UPDATE public.pm_activity_registry 
SET template_insert = 'ha aggiunto membri a {entity}'
WHERE table_name = 'pm_item_assignees';

-- 3. The Logger: Simplified and robust
-- Removed the 10-second silence (it might be too aggressive)
-- Re-implemented the logic that ensures "created" is the fallback if no columns are updated.
...
BEGIN
    ...
    IF _reg.column_templates IS NOT NULL THEN
        FOR _col IN SELECT jsonb_object_keys(_diff_json) LOOP
            ...
            IF _col_template IS NOT NULL THEN
                -- IMPORTANT: On INSERT, we skip logging 'notes' or 'title' as separate updates 
                -- so we can have the generic "created" log instead.
    ...
    -- Generic INSERT log if no specific columns (like Priority or Expiry) were logged during insertion
...
-- 4. EMERGENCY CLEANUP: Remove ANY entry from the last 15 minutes that is clearly wrong or duplicated
DELETE FROM public.pm_activity_logs 
WHERE created_at > NOW() - INTERVAL '15 minutes'
AND (
    (action_type ILIKE 'pm_item_assignees%' AND details->>'description' NOT ILIKE '%aggiunto%')
