...
-- --- PART 4: CLEANUP HISTORY ---
-- Clean double create/update: if there's a specific 'updated:title' or 'updated:notes' log 
-- for the same item at the same time as the 'created' log, remove the 'created' one 
-- OR the updated ones. Prefer 'created' log if it's new.
-- Logic: If we have multiple logs for same item/user at same second, and one is 'created' 
-- and others are 'updated:title'/'updated:notes', delete the UPDATED ones because they are redundant.

DELETE FROM public.pm_activity_logs a USING public.pm_activity_logs b
WHERE a.item_ref = b.item_ref 
AND a.actor_user_ref = b.actor_user_ref 
AND a.created_at = b.created_at
AND b.action_type ILIKE '%:created'
AND (a.action_type ILIKE '%:notes' OR a.action_type ILIKE '%:title')
AND a.id != b.id;
...
DELETE FROM public.pm_activity_logs a WHERE id IN (SELECT id FROM (SELECT id, ROW_NUMBER() OVER (PARTITION BY actor_user_ref, item_ref, details->>'description', created_at::date ORDER BY created_at DESC) as rn FROM public.pm_activity_logs WHERE created_at > NOW() - INTERVAL '24 hours') t WHERE t.rn > 1);
