-- 1. Add client_ref to logs and registry
ALTER TABLE public.pm_activity_logs 
ADD COLUMN IF NOT EXISTS client_ref UUID REFERENCES public.clients(id) ON DELETE CASCADE;

ALTER TABLE public.pm_activity_registry 
ADD COLUMN IF NOT EXISTS client_ref_source TEXT;

-- 2. Update the Logger Function to resolve client_ref
...
-- 3. Seed registry with client_ref_source
UPDATE public.pm_activity_registry SET client_ref_source = 'client_id' WHERE table_name = 'orders';
UPDATE public.pm_activity_registry SET client_ref_source = 'space_ref.ref_ordine.client_id' WHERE table_name IN ('pm_spaces', 'pm_items');
UPDATE public.pm_activity_registry SET client_ref_source = 'pm_item_ref.space_ref.ref_ordine.client_id' WHERE table_name IN ('pm_item_assignees', 'pm_item_links', 'pm_item_comments');
UPDATE public.pm_activity_registry SET client_ref_source = 'space_ref.space_ref.ref_ordine.client_id' WHERE table_name = 'doc_pages';
