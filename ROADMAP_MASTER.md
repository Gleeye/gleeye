...
- **15 maggio 2026** (notte 2): Filone 3A LIVE. Switch a Groq Whisper Turbo (free tier 2h/giorno) + Gemini Flash Lite. Costo ~€0.02 per 30 min audio invece di €0.07. Logging in ai_usage_log. 2 pending strategici emersi e da NON aprire da soli: (1) **Architettura artefatti PM** — dove vivono voice memo/doc/whiteboard/file allegati nella gerarchia commessa→attività→task. (2) **Review qualità report voice memo** — quando avremo volume di test reale. Vedi `pending_pm_artifacts_architecture.md` e `pending_voice_memo_quality_review.md` in memoria.
