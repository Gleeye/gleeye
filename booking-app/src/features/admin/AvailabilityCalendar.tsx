...
function getBlocksForDay(dayOfWeek: number, dateStr: string) {
    ...
    filteredRules.forEach(rule => {
        ...
        // ADD GOOGLE BUSY SLOTS VISUALIZATION
        // We create "fake" rules for busy slots to render them in gray/red
        collabBusy.forEach(busy => {
            result.push({
                id: `busy_${rule.collaborator_id}_${dateStr}_${busy.start}`,
                collaborator_id: rule.collaborator_id,
                day_of_week: dayOfWeek,
                start_time: formatTime(busy.start),
                end_time: formatTime(busy.end),
