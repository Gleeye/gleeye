            ...
            <div className="max-w-xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center mt-10 animate-fade-in">
                ...
                <div className="bg-slate-50/80 rounded-2xl p-6 text-left mb-8 border border-slate-100">
                    <div className="flex justify-between mb-4 border-b border-slate-200 pb-2">
                        <span className="text-slate-500 text-sm font-bold uppercase tracking-wider font-titles">Data</span>
                        <span className="font-bold text-slate-800 font-titles">{selectedSlot?.date}</span>
                    </div>
                    <div className="flex justify-between">
                        <span className="text-slate-500 text-sm font-bold uppercase tracking-wider font-titles">Ora</span>
                        <span className="font-bold text-slate-800 font-titles">{selectedSlot?.time}</span>
                    </div>
                </div>
                <div className="flex flex-col gap-4">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        {/* Google Calendar */}
                        <a
                            href={`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(`Prenotazione: ${selectedService?.name}`)}&details=${encodeURIComponent(`Prenotazione with Gleeye for ${selectedService?.name}.`)}&dates=${format(new Date(`${selectedSlot?.date}T${selectedSlot?.time}:00`), "yyyyMMdd'T'HHmmss")}/${format(addMinutes(new Date(`${selectedSlot?.date}T${selectedSlot?.time}:00`), selectedService?.duration || 60), "yyyyMMdd'T'HHmmss")}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="bg-white border border-slate-200 text-slate-700 font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 hover:bg-slate-50 transition-all hover:shadow-sm font-titles"
                        >
                        ...
                        <div className="space-y-4 animate-fade-in-up">
                            ...
                            <div className="mt-10 flex justify-end">
                                <button
                                    ...
                                    className={`
                                    px-8 py-4 rounded-xl font-bold shadow-xl transition-all flex items-center gap-2 transform active:scale-95 font-titles
                                    ${selectedService
                        ...
                        <div className="space-y-4 animate-fade-in-up">
                            ...
                            <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 p-8">
                                ...
                                <button
                                    ...
                                    className={`
                                    w-full mt-8 py-4 bg-slate-900 text-white rounded-xl font-bold text-lg hover:shadow-xl hover:shadow-blue-500/30 transition-all flex items-center justify-center gap-3 transform active:scale-[0.99] font-titles
